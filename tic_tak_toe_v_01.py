...
def inpuPlayerLetter():
    # Lets the plaer type which letter they want to be.
...
def isWinner(bo,le):
    ...
    (bo[4] == le and bo[5] == le and bo[6] == le) or # acros the middle
    (bo[1] == le and bo[2] == le and bo[3] == le) or # acros the bottom
...
def getComputerMove(board,computerLetter):
    # Given a board and the computer's letter, determine where to move and returns that move.
    if computerLetter =='X':
        playerletter = 'O'
    else:
        playerletter = 'X'

    # Here is our algoritm for our Tic Tac Toe AI:
...
while True:
    #Resset the Board
    theBoard = [' '] * 10
    playerletter,computerLetter = inpuPlayerLetter()

    turn = whoGoesFirst()
    print(' The' + turn + 'ходит первым. ')
    gameIsPlaying = True
    while gameIsPlaying:
        if turn =='Человек':
            # Player's turn.
